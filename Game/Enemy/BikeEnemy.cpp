...
void BikeEnemy::Init(const EnemyGenerateData &GENERATE_DATA, bool DEMO_FLAG)
{
	...
	float lScale = 0.7f;
	model->data.transform.scale = { lScale,lScale,lScale };
	model->data.handle = FbxModelResourceMgr::Instance()->LoadModel(KazFilePathName::EnemyPath + "Bike/" + "BikeEnemy_anim.fbx");	//モデル読み込み
	...
	model->data.transform.pos.y += 7.0f;
...
void BikeEnemy::Update()
{
	...
	*/
	...
	{
		lPos = basePos.z;
		kockBackPos.z = model->data.transform.pos.z - 20.0f;
	}
	/*
	発射
	*/
	...
	{
		//ミサイルの発射口を設定
		misileR.data.transform.pos.x = model->data.transform.pos.x + 5.0f;
		misileR.data.transform.pos.y = model->data.transform.pos.y + 5.0f;
		misileR.data.transform.pos.z = model->data.transform.pos.z + 14.0f;
	...
	*/
	...
	KazMath::Larp(lPos, &model->data.transform.pos.z, lMul);
	//線を登場させる
