...
void DescriptorHeapMgr::CreateBufferView(const short &HANDLE, const D3D12_CONSTANT_BUFFER_VIEW_DESC &BUFFER_VIEW, ID3D12Resource *ADDRESS)
{
...
void DescriptorHeapMgr::CreateBufferView(const short &HANDLE, const D3D12_UNORDERED_ACCESS_VIEW_DESC &BUFFER_VIEW, ID3D12Resource *ADDRESS, ID3D12Resource *COUNTER_RESOURCE)
{
	D3D12_CPU_DESCRIPTOR_HANDLE heapHandle;
	if (isSafeToUseThisHandle(HANDLE))
	{
