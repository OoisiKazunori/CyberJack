...
RWStructuredBuffer<uint2> idData : register(u4);
...
void CSmain(uint3 groupId : SV_GroupID, uint groupIndex : SV_GroupIndex,uint3 groupThreadID : SV_GroupThreadID)
{
    ...
    if(commonData[index].w <= index)
    {
        return;
    }

    uint particleID = 0;
    for(int i = 0;i < meshMaxNum; ++i)
    {
        if(idData[i].x <= index && index <= idData[i].y)
        {
            particleID = i;
            break;
        }
    ...
    //パーティクルの配置--------------------------------------------
    const int PARTICLE_MAX_NUM = commonData[index].z;
    ...
    for(int rayIndex = 0; rayIndex < RAY_MAX_NUM; ++rayIndex)
    {
        ...
        for(int particleIndex = 0; particleIndex < PER_PARTICLE_MAX_NUM; ++particleIndex)
        {
            ...
            const int RANDOM_NUMBER_BIAS = commonData[index].y;
            ...
            outputData[outputIndex].pos = resultPos;
            outputData[outputIndex].color = tex.SampleLevel(smp,uv,0);
            outputData[outputIndex].id = particleID;
        }
    }
    //パーティクルの配置--------------------------------------------
}